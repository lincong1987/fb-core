...
(function (global, factory) {
	...
	Router.prototype.add = function add(routeOptions) {
		var instance = this, newRoutes = [], pathStack = [], routeStack = [], addRoute = function (routeOptions) {
			...
			var parentPath = API.array.last(pathStack), parentRoute = API.array.last(routeStack),
				path = formatPath(routeOptions.path, parentPath), route = {path: path, route: routeOptions},
				params = [];
			API.array.each(path.split(SEPARATOR_PATH), function (item) {
	...
	Router.prototype.parseLocation = function parseLocation(url, callback) {
		...
		// 匹配已注册的 route
		var instance = this, realpathTerms = realpath.split(SEPARATOR_PATH), length = realpathTerms.length,
	...
	Router.prototype.setRoute = function setRoute(location) {
		...
		var newRoute = API.object.copy(linkedRoute), oldRoute = instance.route, oldLocation = instance.location,
			enterRoute = function () {
	...
	var default404 = {
		...
		bind: function bind(node, directive, vnode) {
			...
			var $root = vnode.context.$root || vnode.context, router = $root.$router,
